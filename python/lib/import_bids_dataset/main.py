...
def import_bids_dataset(env: Env, args: Args, legacy_db: Database):
    ...
    for subject in bids.subjects:
        participant_row = (
            bids.tsv_participants.get_row(subject.label)
            if bids.tsv_participants is not None
            else None
        )

        subject_infos.append(BidsSubjectInfo(subject.label, participant_row))

        for session in subject.sessions:
            session_infos.append(BidsSessionInfo(subject.label, participant_row, session.label))

    check_or_create_bids_subjects(env, subject_infos, args.create_candidate)
    sessions = check_or_create_bids_sessions(env, session_infos, args.create_session)
...
def import_bids_eeg_data_type_files(
    ...
    Eeg(
        env                    = env,
        bids_layout            = data_type.root_dataset.layout,
        bids_info              = BidsDataTypeInfo(
            data_type.subject.label,
            None,
            data_type.session.label,
            data_type.name,
        ),
        db                     = legacy_db,

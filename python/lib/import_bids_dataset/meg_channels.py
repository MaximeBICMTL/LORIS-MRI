...
def read_meg_channels(env: Env, physio_file: DbPhysioFile, acquisition: BidsMegAcquisition):
    name = acquisition.ctf_path.name

    bids_path = BIDSPath(
        subject=acquisition.subject.label,
        session=acquisition.session.label,
        ...
        datatype='meg',
        root=acquisition.root_dataset.path,
        suffix='meg',
        extension='.ds',
    )
    ...
    for ch in raw.info['chs'][:5]:  # First 5 channels
        ...
        if not (math.isnan(loc_x) or math.isnan(loc_y) or math.isnan(loc_z)):
            ...
            electrode = DbPhysioElectrode(
                ...
                impedance=None,
                file_path=acquisition.channels.path if acquisition.channels else None,
            )

...
def read_and_insert_bids(
    ...
    for subject_reader in bids_reader.subjects:
        subject_infos.append(BidsSubjectInfo(subject_reader.subject, subject_reader.participant_row))
        for session_reader in subject_reader.sessions:
            session_infos.append(BidsSessionInfo(
                session_reader.subject,
                session_reader.participant_row,
    ...
    for data_type_reader in bids_reader.data_types:
        visit_label = data_type_reader.session if data_type_reader.session else default_bids_vl
        loris_bids_data_type_rel_dir = os.path.join(
            f'sub-{data_type_reader.subject}',
            f'ses-{visit_label}',
            data_type_reader.data_type,
        )

        if not nocopy:
            lib.utilities.create_dir(os.path.join(loris_bids_root_dir, loris_bids_data_type_rel_dir), verbose)

        candidate = try_get_candidate_with_cand_id(env.db, data_type_reader.subject)
        if candidate is None:
            candidate = try_get_candidate_with_psc_id(env.db, data_type_reader.subject)

        session_label = try_get_session_with_cand_id_visit_label(env.db, candidate.cand_id, visit_label)

        match data_type_reader.data_type:
            case 'eeg' | 'ieeg':
                Eeg(
                    env,
                    bids_layout   = bids_reader.layout,
                    session       = session_label,
                    bids_info     = BidsDataTypeInfo(
                        data_type_reader.subject,
                        data_type_reader.participant_row,
                        data_type_reader.session,
                        data_type_reader.data_type,
                    ),
                    db            = db,
                    data_dir      = data_dir,
            ...
            case 'anat' | 'dwi' | 'fmap' | 'func':
                Mri(
                    ...
                    bids_layout   = bids_reader.layout,
                    session       = session_label,
                    bids_sub_id   = data_type_reader.subject,
                    bids_ses_id   = data_type_reader.session,
                    bids_modality = data_type_reader.data_type,
                    db            = db,
                    verbose       = verbose,
                    data_dir      = data_dir,
            ...
            case _:
                print(f"Data type {data_type_reader.data_type} is not supported. Skipping.")

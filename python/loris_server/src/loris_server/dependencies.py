...
from lib.user import is_user_account_active, is_user_account_locked
...
def get_user(env: EnvDep, request: Request) -> DbUser:
    ...
    try:
        payload = jwt.decode(  # type: ignore
            ...
            algorithms=['HS256'],
            options={
                # Standard PHP JWT claims.
                'require': ['aud', 'exp', 'iat', 'iss', 'nbf', 'user'],
                # The LORIS PHP JWT implementation does not validate audience, which is therefore
                # also disabled here.
                'verify_aud': False,
            },
        )
    ...
    if not isinstance(issuer, str) or issuer == '' or audience != issuer:
        raise _unauthorized("Login information is incorrect.")
    ...
    if user is None or not is_user_account_active(env, user):
        raise _unauthorized("Login information is incorrect.")
